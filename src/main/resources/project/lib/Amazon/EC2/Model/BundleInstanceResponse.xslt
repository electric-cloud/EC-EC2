...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ec2="http://ec2.amazonaws.com/doc/2010-06-15/" exclude-result-prefixes="ec2">
	...
	<xsl:template match="BundleInstanceResponse">
		<xsl:element name="BundleInstanceResponse" namespace="{$ns}">
			...
			<xsl:element name="BundleInstanceResult" namespace="{$ns}">
				<xsl:apply-templates select="bundleInstanceTask" />
			</xsl:element>
		</xsl:element>
	</xsl:template>
	<xsl:template match="bundleInstanceTask">
		...
		<xsl:element name="Storage" namespace="{$ns}">
			<xsl:element name="S3" namespace="{$ns}">
				...
				<xsl:element name="AWSAccessKeyId" namespace="{$ns}">
					<xsl:value-of select="S3/awsAccessKeyId" />
				</xsl:element>
				<xsl:element name="UploadPolicy" namespace="{$ns}">
					<xsl:value-of select="S3/uploadPolicy" />
				</xsl:element>
				<xsl:element name="UploadPolicySignature" namespace="{$ns}">
					<xsl:value-of select="S3/uploadPolicySignature" />
				</xsl:element>
			</xsl:element>
		</xsl:element>
	</xsl:template>
