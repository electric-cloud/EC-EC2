...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="DescribeVpnConnectionsResponse">
		<xsl:element name="DescribeVpnConnectionsResponse"
			...
			<xsl:element name="DescribeVpnConnectionsResult"
				namespace="{$ns}">
				<xsl:apply-templates select="vpnConnectionSet" />
			</xsl:element>
		</xsl:element>
	</xsl:template>
	<xsl:template match="vpnConnectionSet">
		<xsl:for-each select="item">
			<xsl:element name="VpnConnection" namespace="{$ns}">
				<xsl:element name="VpnConnectionId" namespace="{$ns}">
					<xsl:value-of select="vpnConnectionId" />
				</xsl:element>
				...
				<xsl:element name="CustomerGatewayConfiguration"
					namespace="{$ns}">
					<xsl:value-of select="customerGatewayConfiguration" />
				</xsl:element>
