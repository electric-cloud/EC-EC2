...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ec2="http://ec2.amazonaws.com/doc/2010-06-15/" exclude-result-prefixes="ec2">
    ...
    <xsl:template match="StopInstancesResponse">
        <xsl:element name="StopInstancesResponse" namespace="{$ns}">
            <xsl:element name="ResponseMetadata" namespace="{$ns}">
                <xsl:element name="RequestId" namespace="{$ns}">
                    <xsl:value-of select="requestId"/>
                </xsl:element>
            </xsl:element>
            <xsl:element name="StopInstancesResult" namespace="{$ns}">
    ...
    <xsl:template match="instancesSet">
        <xsl:for-each select="item">
            <xsl:element name="StoppingInstances" namespace="{$ns}">
