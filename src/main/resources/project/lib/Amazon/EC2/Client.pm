...
#
    ...
    sub new {
        ...
        my $defaultConfig =  {
                            ServiceURL => "http://localhost/",#ServiceURL => "https://ec2.amazonaws.com",
    ...
    sub createTags {
        my ($self, $request) = @_;
        if (not ref $request eq "Amazon::EC2::Model::CreateTagsRequest") {
            require Amazon::EC2::Model::CreateTagsRequest;
            $request = Amazon::EC2::Model::CreateTagsRequest->new($request);
        }
        require Amazon::EC2::Model::CreateTagsResponse;
        return Amazon::EC2::Model::CreateTagsResponse->fromXML($self->_invokeVer($self->_convertCreateTags($request), "2013-10-15"));
    }
    ...
    sub _invoke {
        ...
        eval {
            ...
            my $xslt = XML::XSLT->new ($xsl_string, warnings => 1);
            $xslt->transform ($response->content);
            $results = $xslt->toString;
            #print $results;
            $xslt->dispose();
        };
        if ($@) {
    ...
    #
    # Invoke request for specified api version
    #
    sub _invokeVer {
        my ($self, $parameters, $version) = @_;
        my $actionName = $parameters->{Action};
        my $response = undef;
        my $statusCode = 200;
