    ...
    sub CreateVolume {
        ...
        foreach (@instList) {
            ...
            eval {
                ...
                mesg( 1,
                    "Creating volume from snapshot in zone $actualZone\n" );
...
"New volume $newvol created from snapshot $snap_id for instance $id\n"
                    ...
                    $opts->{pdb}->setProp( "$propResult/Instance-$id/NewVolume",
                        $newvol );
...
"Volume $vol was not attached to instance $instance ($id).\n"
    ...
    sub MOCK_SnapVolume {
        ...
        $snap_id = "snap-$r";
        $opts->{pdb}
          ->setProp( "$::gMockRegistry/Snapshots/$snap_id", "created" );
        mesg( 1, "Created new snapshot $snap_id\n" );
    ...
    sub MOCK_API_DescribeInstances {
        ...
        foreach my $instanceName (@instances) {
            ...
            $resultHash->{$instanceName}{key} =
              $opts->{pdb}
              ->getProp("$::gMockRegistry/Instances/$instanceName/key");
            $resultHash->{$instanceName}{type} =
              $opts->{pdb}
        ...
        foreach (@volumeList) {
            ...
            while (1) {
                ...
                if ( $status eq "in-use" ) {
                    ...
                    eval {
                        my $request =
                          new Amazon::EC2::Model::DetachVolumeRequest(
                            { "VolumeId" => "$vol_id" } );
                ...
                mesg( 1,
                    "Waiting for volume $vol_id to be in available state\n" );
                sleep 10;
            }
            if ( !$detachOnly ) {
                ## delete volume
    ...
    sub MOCK_API_DeleteVol {
        ...
        foreach (@volumeList) {
            ...
            my $instance =
              $opts->{pdb}
              ->getProp("$::gMockRegistry/Volumes/$vol_id/instance");
    ...
    sub _terminate_instance {
        ...
        eval {
            ...
            mesg( 1,
                "Can't terminate instance $instance_id :" . Dumper $@ . "\n" );
            return 0;
        };
        return 1;
    }
...
# If the first letter is "i" then we only want to terminate a specific instance or list of instances
    ...
    sub DeregisterInstance {
        ...
        my $request = new Amazon::EC2::Model::DeregisterImageRequest(
            { "ImageId" => "$ami" } );
            ...
            eval {
                ...
                if ( $response->isSetDescribeImagesResult() ) {
                    my $describeImagesResult =
                      $response->getDescribeImagesResult();
...
"Running $count instance(s) of $ami in zone $zone as type $instanceType with group $group\n"
        ...
        eval {
            ...
            if ( $response->isSetRunInstancesResult() ) {
                ...
                mesg( 1,
                    "Run instance returned reservation id $reservation\n" );
            }
        };
        if ($@) { throwEC2Error($@); }
                ...
                if ( $response->isSetDescribeInstancesResult() ) {
                    ...
                    foreach (@$resList) {
                        ...
                        foreach (@$instanceList) {
                            ...
                            mesg( 1,
                                "Evaluating instance $id in state $state\n" );
                            $total += 1;
...
"WARNING: Can't create resource because of no public IP was assigned to the created instance\n"
                        ...
                        if ( "$poolName" ne "" ) {
                            mesg( 1,
                                "Poolname is not empty, adding resources." );
                            $resource =
                            ...
                            $ec->createProperty( "$p_path/etc/",
                                { propertyType => 'sheet' } );
                            $opts->{pdb}
                              ->setProp( "$p_path/createdBy", "EC-EC2" );
                        ...
                        if ( "$propResult" ne "" ) {
                            $opts->{pdb}
                              ->setProp( "$propResult/Instance-$id/AMI",
                                "$image" );
                            $opts->{pdb}
                            ...
                            $opts->{pdb}
                              ->setProp( "$propResult/Instance-$id/Private",
                                "$privateIP" );
                            $opts->{pdb}
                              ->setProp( "$propResult/Instance-$id/Zone",
                                "$actualZone" );
                            $opts->{pdb}
    ...
    sub getInstancesForTermination {
        ...
        eval {
            ...
            if ($res) {
                ...
                $data =
                  $xml->{response}->{resourcePool}->{resourceNames}
                  ->{resourceName};
            }
        };
        if ( !$data ) {
